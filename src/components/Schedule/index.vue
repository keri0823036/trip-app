  ...
  <div class="schedule">
    <template v-if="isLoaded">
      <div>
        ...
        <h3 class="schedule__title">行程表</h3>
        <ul class="schedule__list">
          <template v-for="item in currentSchedule">
            <li :key="item.id" class="schedule__list__item">
...
export default {
  ...
  computed: {
    currentSchedule() {
      ...
      const currentKey = Object.keys(this.scheduleData).find(key => key.includes(this.sheetName))

      return this.scheduleData[currentKey]?.data
    }
  },
  watch: {
    scheduleData(data) {
  ...
  methods: {
    getIcon(type) {
      return spotsTypeConfig[type].icon
    },
    copyAddress (text) {
      const vueThis = this
